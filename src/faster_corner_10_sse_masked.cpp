...
namespace fast
{
  ...
  void faster_corner_detect_10_masked(
    const fast_byte* img,
    const int img_width,
    ...
    std::vector<fast_xy>& corners)
  {
      ...
      const int occ_stride=(img_width/occupancy_cell_size+1);
      ...
      {
          const std::uint8_t* occ = &occupancy[(y/occupancy_cell_size)*occ_stride];
      ...
      std::vector<fast_xy>& corners)
  {
      ...
      if (is_aligned<16>(img) && is_aligned<16>(img+img_stride))
          faster_corner_detect_10_masked<true>(
                img, img_width, img_height, img_stride, barrier,
                occupancy, occupancy_cell_size, corners);
      else
          faster_corner_detect_10_masked<false>(
                img, img_width, img_height, img_stride, barrier,
                occupancy, occupancy_cell_size, corners);
  }
}
